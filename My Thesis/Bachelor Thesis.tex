...
\\
\newline \noindent At the end the system usability scale (SUS) score had to be calculated to measure the usability of the application. SUS is considered one of the most used questionnaires to measure usability and is considered as a reliable tool for measuring usability by consisting of a 10 item questionnaire and responses ranging from one to five in which one represents strongly disagree and five represents strongly agree.
\\
\newline \noindent So the SUS score for the application was calculated from the results explained above by collecting the average scores of all the participants and then for the positively-worded items (1,3,5,7 and 9) the score was the number users chose - one and the rest of the questions (2,4,6,8 and 10) it was five - the number the user chose then summing them all and multiplying by 2.5 at the end it gave us a SUS score of 76 so according to the graph below~\ref{fig:SusScore}\footnote[1]{http://www.measuringusability.com/sus.php} it corresponds to a B grade.
