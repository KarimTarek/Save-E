...
Energy is now one of the main concerns the world is concerned about , since energy resources are running out and people's energy consumption is increasing dramatically it was expected that from 2008 to 2030 world energy consumption is expected to increase more than 55\%\footnote{http://facts.randomhistory.com/energy-facts.html}. Moreover energy consumption increase the rate of Carbon dioxide (CO2) in our environment which then leads to global warming and other environmental problems.
\\
...
currently the public display has three users logged-in and another user wants to connect so as you can see in figure~\ref{fig:Example1} the Boolean array which represents the current busy slots of the screen indicates that slot 1,2 and 3 are taken.
...
\\
\newline \noindent The second question involved complexity and the users were asked to give a rating from one to five on how much do they see the system complex. The average result was 2.1 which is nearly that the users don't agree with the concept that the system is complex.
\\
...
\\
\newline \noindent So the SUS score for the application was calculated from the results explained above by collecting the average scores of all the participants and then for the positively-worded items (1,3,5,7 and 9) the score was the number users chose - one and the rest of the questions (2,4,6,8 and 10) it was five - the number the user chose then summing them all and multiplying by 2.5 at the end it gave us a SUS score of 76 so according to the graph below in figure~\ref{fig:SusScore}\footnote[1]{http://www.measuringusability.com/sus.php} it corresponds to a B grade.
