...
\\
\newline \noindent As for smart phones they have lately become an essential part of people's lives. People use their mobile phones now to do everything in their lives like checking e-mails, chatting with each other, taking photos and many other applications. As a result smart phones were involved in the application.
...
\\
\newline \noindent Moreover, helping the user in saving energy in their daily lives by giving them tips about the actions they usually do every day without even knowing that it wastes energy, also giving them alternatives to do them. This might be one of the most important ways to change people's attitude towards the energy consumption case. Of course this helps if not only by decreasing the energy the user consume, but also by letting the user know ways about how the energy can be wasted without him knowing. And instead of doing the task in x amount of energy, the user can end up using half of this amount.
\\
\newline \noindent The system had to be trusted, robust and user friendly so in order to try to achieve that, privacy profiles were implemented to increase user trust and confidence in the application by giving the user an option to hide his data if someone suddenly appears or even if he doesn't want anyone to check his consumption. Finally we wanted to take advantage of the multi-user setup to give users an option to view charts together in order to compare their energy consumption with each other which is believed to motivate them to save more energy.
%\section{Outline}
%\label{sec:Outline}
...
Synchronization of the displays was done by also using the connections as mentioned in the Connections section \ref{sec:Connection}. So first when the user presses any button on his mobile to navigate the system on the display the mobile pops-up the loading widget which tells the user what the server is trying to do in that moment. While the request reaches the server, the server sends to the display what to show. Secondly, an event is triggered on the channel which the user is connected to with a loading complete header which indicates that the request has successfully executed and the loading widget is removed from the user mobile screen then the page is changed to match the display contents. Here is an example in figure~\ref{fig:Example7} to illustrate more: 
...
\newline \noindent This was made by implementing css classes for each element on the screen for each possible scenario for example when a user navigates on the display alone he is taking the whole screen alone and all the elements are given the css for that situation. However, when another user signs-in he triggers a function which then knows the page that each user is currently at and begin to change the css of each of the elements to the new situation, so the css is all checked and changed when a user connects to the display and other users are already on the display. Moreover, when a user navigates from one page to another the same function is called but only to correct the css of any change happened and to maintain the correct user interface for all the users.

\section{User Data}
\label{sec:UserData}
User data which is simply represented as charts in the project and letting the user know their energy consumption was one of the objectives to increase energy awareness and help users save energy. Therefore user data had to be made in order to illustrate to the user his consumption. In addition helping in two points:
\\
\newline \noindent The first point: user data helps the user to know their detailed energy consumption and as a result the user can know where does the problem arise or from which device he consumes energy the most. This part is represented as two different types of charts. To begin with, the first chart is placed on user's profiles and shows their total energy consumption as a pie chart including each device consumption in percentages as you can see in figure~\ref{fig:Example15}. So to give the user an overview of what his total energy consumption might be, the charts were placed on the profile to allow the users to check them once they login to the system since its the first thing they view once they login.
...
Privacy was one of the important aspects of the project since the project was not about creating an application to motivate people to save energy and increase their energy awareness only, but also a robust and trusted system. Therefore, privacy had to be implemented in the project in order to add the option of hiding the detailed consumption and viewing it on the mobile screen instantaneously so that the user can use this option whenever he wants, which makes the user feel more safe. For this reason two privacy modes were created. First the high privacy mode which orders the system to hide the data from the public display and sends it to the mobile. Consequently only the user is able to see his data on the mobile. Secondly the low privacy mode which is the normal mode, the data is kept at the display available for all users to see normally.
\\
\newline \noindent However, certain constraints arose while trying to implement the privacy option:
\newline \noindent First the size limit of pusher service used for the connection between the public display and the mobile was only 10KB thus sending the whole page to the mobile from the screen was not an option.
\newline \noindent Secondly the css of the page on the public display was implemented using twitter bootstrap as mentioned in section \ref{sec:TechnologiesUsed} as for the mobile, Jquery mobile was used. since the two technologies were different, the data couldn't be sent directly to the mobile as this might lead to the replacement of styles.
\\
\newline \noindent The two problems were solved by just sending the crucial information which was the user consumption data in this case. As it already appears on the leader board and they are made to let people compete with each other, not to hide it. So the charts' engine used was put on the mobile and the data was the only part sent. Therefore the first problem, which was the size of the file transferred, was solved. The second problem was also solved because no styles or css files  are required to be sent with the data. The mobile then passes the data to the charts engine to render it and generate the charts once the user chooses the high privacy option. As you can see in figure~\ref{fig:Example8}, the figure shows the application with the low privacy profile setting and figure~\ref{fig:Example9} with the high privacy profile setting.
\begin{figure}[H]
\centring
...
Gamification or applying game mechanics is now widely used in many applications because not only it enhances the user experience with the application and involves more fun but also it motivates users to challenge and compete against each other in order to be from the top three or the top ten players. As a result this concept was used in the project bringing more interaction between the users and increasing their motivation to compete against each other to save energy and gain badges and virtual money to be able to make their way up the leader board as it will be explained later in this section.
...
\\
\newline \noindent At first users have a certain amount of money by default then when they start measuring their energy consumption day by day the cost is calculated and deducted from the money they have so the more you consume energy the less the money you have and vice versa. Also when the system detects that the data is for a new month the system adds new money to the users so when the users save more energy and pay less, their money increase and when they reach a certain amount of money which is decided by the system, they gain badges which leads to a level up and improving their ranks on the leader board.
\\
\newline \noindent Now lets have a deeper look on the game mechanics within the application. To begin with, the levels are shown on the users' profile page. Once the user logs in levels are shown exactly at the top of the screen as in figure~\ref{fig:Example12}.
...
\newline \noindent Virtual money is an important factor also in the application game mechanics it represents the points that each user has which also acts as an indication on how much energy the user is saving, the higher the user's consumption the lower virtual money he has because the cost of the user consumption is deducted from the money he has. Money also is displayed at the top of the users' profiles as shown in figure~\ref{fig:Example12}.
...
\\
\newline \noindent Finally leader board is the place where all the users can see the level, money and badges of each other which also adds to the competitive environment in a way that all users are motivated to be from the first three rankings or in other words the top energy savers. Moreover, it allows users to check their ranking with other users using the application just to give the users an idea on their energy consumption related to the people around them. As you can see in figure~\ref{fig:Example14} the leader board is shown with users sorted by their levels also showing their badges and money.
...
To sum up the target was to create a reliable and trusted system using modern ways such as public displays and smartphones to attract more people. Moreover, trying to solve the energy consumption issue which is considered a global problem. We were motivated by the many ideas mentioned in different papers related to the interaction of users on the display and related to interactive public displays also how to display personalized content on it. A system was built taking into consideration all the recommendations found in papers with the resources available which keep track of users energy usage and allowing them to check their energy usages regularly also it gives users tips on small actions they do everyday in order to change their attitude towards this things that wastes a lot of energy. Furthermore, the advantage of multi-user architecture and game mechanics were taken into consideration, in order to create a competitive environment for users to compete in gaining more virtual money and gaining badges to level up, which corresponds to saving more energy and in the same time providing privacy profiles for users to choose between them.

\section{Future Work}
\label{sec:FutureWork}
As for future work, the application needs to be fully integrated with real user devices instead of dummy data also a lot of features can be added as alerts for the users when they reach a certain amount of energy consumption for a certain device or for their total energy usage that the user themselves set. Also replacing the online service pusher which gives you the option to put realtime events in you application by an offline service so the user does not need to access the internet and can use the application while being offline.
\\
\newline \noindent Moreover implementing a semaphore-like mechanism to prevent the shared variables which in this case are the number of users stored by the browser which can cause problems in knowing the actual number of users and depending on it, the layout could be broken. Furthermore, users manipulate a shared variable while connecting to the system which is represented in a default channel that all the users connect through to the server before their data is displayed on the screen, if two or more users connect at the same exact time, conflicts occur between requests.
\\
\newline \noindent Finally a more responsive layout can be implemented in order to help users feel more comfortable with the system (for example, with more animations) since from my point of view user interface is one of the most important factors that affects any product nowadays.
...
The following are the introduction, task and questionnaire questions that was given to the users to evaluate the system.

\begin{figure}[h]
\centring
\includegraphics[width=1\textwidth]{Intro.png}
\caption{This figure shows the introduction that was given to the users while evaluating the system.}
\label{fig:Intro}
\end{figure}
...
\caption{This figure shows the tasks given to the users .}
\label{fig:Tasks}
\end{figure}
...
\caption{This figure shows the first page of the questionnaire.}
\label{fig:Page1}
\end{figure}
...
\caption{This figure shows the second page of the questionnaire.}
\label{fig:Page2}
\end{figure}

\begin{figure}[H]
\centring
\includegraphics[width=1\textwidth]{Page3.png}
\caption{This figure shows the third page of the questionnaire.}
\label{fig:Page3}
\end{figure}
